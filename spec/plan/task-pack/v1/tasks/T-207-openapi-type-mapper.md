...
| 测试类型 | 测试内容 | 预期结果 |
|---------|---------|---------|
| 单元测试 | String 类型映射 | `{type: string}` |
| 单元测试 | Amount 类型映射 | `{type: string, format: decimal}` |
| 单元测试 | Date 类型映射 | `{type: string, format: date}` |
| 单元测试 | 数组类型映射 | `{type: array, items: {$ref}}` |
| 单元测试 | 对象类型映射 | `{$ref: './ClassName.yaml'}` |
| 单元测试 | 必填字段收集 | optionality="M" 的字段在 required 中 |
| 单元测试 | maxLength 生成 | length 字段正确映射 |
| 单元测试 | default 生成 | defaultValue 正确映射 |
| 单元测试 | transitory 过滤 | groupId/occurrenceCount 被排除 |
| 单元测试 | maxItems 解析 | "0..9" -> maxItems: 9 |
| 单元测试 | maxItems 无限制 | "0..N" -> 无 maxItems |
| 单元测试 | 完整 Object Schema | type, required, properties 完整 |
...
class OpenApiTypeMapperTest {
    ...
    @Test
    void shouldMapAmountWithDecimalFormat() {
        FieldNode field = FieldNode.builder()
            .dataType("Amount")
    ...
    void shouldMapArrayType() {
        ...
        assertEquals("./CBACardArray.yaml", items.get("$ref"));
    }

    @Test
    void shouldMapObjectAsRef() {
        FieldNode field = FieldNode.builder()
            .isObject(true)
    ...
    @Test
    void shouldCollectRequiredFields() {
        List<FieldNode> fields = Arrays.asList(
            FieldNode.builder().camelCaseName("required1").optionality("M").build(),
            FieldNode.builder().camelCaseName("optional1").optionality("O").build(),
            FieldNode.builder().camelCaseName("required2").optionality("M").build()
        );
    ...
    @Test
    void shouldFilterTransitoryFields() {
        List<FieldNode> fields = Arrays.asList(
    ...
    void shouldIncludeMaxLength() {
        ...
        assertEquals(30, schema.get("maxLength"));
    }
...
| 风险 | 影响 | 缓解措施 |
|------|------|---------|
| dataType 拼写变体 | 映射失败 | 大小写忽略、添加别名 |
| occurrenceCount 格式不标准 | maxItems 解析失败 | 容错处理，返回 null |
| className 为空 | $ref 路径无效 | 使用 camelCaseName 回退 |
| 循环引用 | OpenAPI 验证失败 | 本组件不处理，由 T-303 验证 |
| 超长字段名 | YAML 格式问题 | 无影响（YAML 支持长键名） |
| 特殊字符 | 路径包含非法字符 | T-106 已处理命名规范化 |
| required 空列表 | 不必要的空数组 | 仅非空时添加 required 键 |
