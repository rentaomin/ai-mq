...
8. [ ] 单元测试覆盖所有映射规则
9. [ ] 单元测试覆盖边界情况（null、空字符串）

## Tests

| 测试类型 | 测试内容 | 预期结果 |
|---------|---------|---------|
| 单元测试 | String dataType 映射 | 返回 "String" |
| 单元测试 | Amount dataType 映射 | 返回 "java.math.BigDecimal" |
| 单元测试 | 数组字段映射 | 返回 "java.util.List<ClassName>" |
| 单元测试 | 对象字段映射 | 返回 className |
| 单元测试 | 未知 dataType | 返回 "String" |
| 单元测试 | null dataType | 返回 "String" |
| 单元测试 | import 收集 | 正确收集 BigDecimal、List |
| 单元测试 | 多字段 import 收集 | 无重复、按字母排序 |
| 单元测试 | 简单类型名提取 | List<X> -> List<X> |
...
class JavaTypeMapperTest {
    ...
    @Test
    void shouldMapAmountToBigDecimal() {
        FieldNode field = FieldNode.builder()
            .dataType("Amount")
            .build();
    ...
    @Test
    void shouldMapArrayToList() {
        FieldNode field = FieldNode.builder()
            .isArray(true)
            .className("CBACardArray")
            .build();

        assertEquals("java.util.List<CBACardArray>", mapper.mapType(field));
    }

    @Test
    void shouldMapObjectToClassName() {
        FieldNode field = FieldNode.builder()
            .isObject(true)
            .className("CreateApplication")
            .build();

        assertEquals("CreateApplication", mapper.mapType(field));
    }

    @Test
    void shouldReturnStringForUnknownType() {
        FieldNode field = FieldNode.builder()
    ...
    @Test
    void shouldReturnStringForNullType() {
        FieldNode field = FieldNode.builder()
            .dataType(null)
            .build();
    ...
    @Test
    void shouldCollectImportsForBigDecimal() {
        FieldNode field = FieldNode.builder()
            .dataType("Amount")
            .build();
    ...
    @Test
    void shouldCollectImportsForList() {
        FieldNode field = FieldNode.builder()
            .isArray(true)
            .className("Item")
            .build();
...
| 风险 | 影响 | 缓解措施 |
|------|------|---------|
| dataType 拼写不一致 | 映射失败 | 统一大小写处理、添加别名 |
| className 为空 | 生成无效类型 | 回退到 camelCaseName |
| 循环引用 | 类型解析死循环 | 本组件不处理循环，由 T-204 处理 |
| BigDecimal 精度 | 金额计算问题 | 文档说明使用场景 |
| 泛型擦除 | 运行时类型信息丢失 | 不影响代码生成 |
| 特殊字符 | 类名包含非法字符 | T-106 已处理命名规范化 |
